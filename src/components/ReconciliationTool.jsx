// ============================================================================
// 🎯 VERSIÓ v34 PENDING FILTERS & STICKY BAR - 17 Gener 2026
// ============================================================================
// - ✅ NEW: Botó "Només Pendents" a les taules de Factures i Banc
// - ✅ FIX: La barra de conciliació manual ara és STICKY (es queda fixa al fer scroll)
// - ✅ Manté tota la funcionalitat anterior (Cerca, Signes, Delete, Ordre Smart...)
// ============================================================================
...
export default function ReconciliationTool() {
  console.log('🎯 ReconciliationTool v34 PENDING FILTERS');
  ...
  // ESTATS DE CERCA I FILTRE
  const [invoiceSearch, setInvoiceSearch] = useState('');
  const [bankSearch, setBankSearch] = useState('');
  const [showPendingInv, setShowPendingInv] = useState(false); // Nou filtre factures
  const [showPendingBank, setShowPendingBank] = useState(false); // Nou filtre banc
  ...
  // --- FILTRES (AMB NOU FILTRE PENDENTS) ---
  const filteredInvoices = useMemo(() => {
    return invoices.filter(inv => {
      ...
      // 3. ⚠️ FILTRE PENDENTS (NOU)
      if (showPendingInv) {
        const rIdx = invoices.indexOf(inv); // Index real
             ...
             <div className="flex items-center gap-4">
               ...
               <div className="relative">
                 ...
                 <input type="text" placeholder="Cercar..." value={invoiceSearch} onChange={e => setInvoiceSearch(e.target.value)}
                   className="pl-8 pr-2 py-1 text-[10px] rounded bg-slate-700 border-none text-white placeholder-slate-400 outline-none focus:ring-1 focus:ring-slate-500 w-32"/>
               </div>
                ...
                {filteredInvoices.map((inv, i) => {
                  ...
                  return (
                    <tr key={i} onClick={() => !isOK && setSelectedInvIndices(prev => { const n=new Set(prev); n.has(rIdx)?n.delete(rIdx):n.add(rIdx); return n; })} 
                      ...
                      <td className={`p-2 text-right font-black ${amt < 0 ? 'text-rose-600' : 'text-indigo-700'}`}>{safeGet(inv, COL_FAC_TOTAL)}€</td>
